...
Reinforcement Learning has its root in many fields such as psychology, neuroscience, economics, mathematics, engineering and computer science. One such example is, suppose the goal of the agent(human in this case) is to be happy. Agent will look for different way, taking different actions from his current state(policy) so as to change the mood to be in happy state. On changing state, the agent may receive a reward(release of the neurotransmitter dopamine). How happy the agent is, will be determined by the total reward received.
